...
const Blogs = ({entries}: InferGetStaticPropsType<typeof getStaticProps>) => {
  ...
  entries.items.forEach((e:Entry<any>) => {
  ...
  return (
      <>
        ...
        <p className={styles.codeBody}>
          So you've come across my personal blog, Welcome! The content here is mainly broken into two portions;
          content around heinous short stories while written in one go sunken into my couch, and developer content (lots of Looker and GCP content)
        ...
        <div className={styles.footerRow}>
          {searchValue && <p className="blogRowText" onClick={() => setSearchValue('')}>X</p>}
          {distinctTags && Array.from(distinctTags).map((t,key) =>
          // 
            <p key={Math.random()} className="blogRowText" onClick={(e) => {
        ...
        {!searchValue && (developerBlogs.map((v:Entry<any>,k) => 
            ...
            <Image alt='' className={styles.imageStyleCard} src={`https:${v.fields.thumbnail.fields.file.url}`} width={v.fields.thumbnail.fields.file.details.image.width} height={v.fields.thumbnail.fields.file.details.image.height}/>
            <Link href={`/blogs/developer/${v.sys.id}`} passHref>
        ...
        {!searchValue && (shortStoryBlogs.map((v:Entry<any>,k) => 
            ...
            <div className={styles.card}>
            <Image alt='' className={styles.imageStyleCard} src={`https:${v.fields.thumbnail.fields.file.url}`} width={v.fields.thumbnail.fields.file.details.image.width} height={v.fields.thumbnail.fields.file.details.image.height}/>
            <Link href={`/blogs/short-story/${v.sys.id}`} passHref>
