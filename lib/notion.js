...
const getPageInfo = async () => {
    ...
    await Promise.all(results.map(async (p) => {
        ...
        try {
            ...
            pageData['block_id'] = p.id ? p.id : null
            pageData['created_time'] = p.created_time ? p.created_time : ''
            pageData['properties'] = p.properties ? p.properties : {}
            pageData['url'] = p.url ? p.url : ''
            pageData['blocks'] = pageBlocksData ? pageBlocksData : []
...
export const projectDetails = async () => {
    ...
    // remove undefined because getStaticProps can't serialize undefined
    const completedProj = data
        .filter((p) => p.properties.Status.select?.name === 'Completed')
    const inProgressProj = data
        .filter((p) => p.properties.Status.select?.name === 'In Progress')
    const nextUpProj = data
        .filter((p) => p.properties.Status.select?.name === 'Next Up')
    const noStatusProj = data
        .filter((p) => p.properties.Status.select?.name === undefined)

    return {'completed': completedProj,'inProgress': inProgressProj, 'nextUp': nextUpProj,'noStatus': noStatusProj}
  }